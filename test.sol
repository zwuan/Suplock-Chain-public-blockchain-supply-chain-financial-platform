...
contract SampleERC865Token is ERC865Token {
  ...
  function transferFromPlatform(address _to, uint256 _amount) public onlyOwner returns (bool) {
  ...
  function transferToPlatform(address _from, uint256 _amount, uint _fee) public onlyOwner returns (bool) {
  uint total = _amount.add(_fee);
  require(total <= balances[_from]);
  balances[_from] = balances[_from].sub(total);
  balances[owner] = balances[owner].add(total);
  emit Transfer(_from, owner, total);
  return true;
  }
}
